...
def get_root_dir():
    ...
    # Assumes utils.py is in src/utils/
    return os.path.abspath(os.path.join(os.path.dirname(__file__), '..', '..'))
def load_llm_preferences():
    """Loads LLM preferences from the JSON file in the root directory."""
    file_path = os.path.join(get_root_dir(), LLM_PREFS_FILE)
    logger.info(f"Attempting to load LLM preferences from: {file_path}") # Add logging

    # --- Set new defaults ---
    defaults = {
        "last_provider": "google",
        "last_model": "gemini-2.0-flash",
        "api_keys": {} # Store API keys per provider
    }
    # --- End new defaults ---

    if not os.path.exists(file_path):
        logger.warning(f"LLM preferences file not found. Using defaults.")
        return defaults["last_provider"], defaults["last_model"], defaults["api_keys"]

    try:
        with open(file_path, 'r') as f:
            prefs = json.load(f)
        # Ensure all expected keys are present, using defaults if missing
        loaded_provider = prefs.get("last_provider", defaults["last_provider"])
        loaded_model = prefs.get("last_model", defaults["last_model"])
        loaded_api_keys = prefs.get("api_keys", defaults["api_keys"])

        logger.info(f"LLM preferences loaded successfully.")
        return loaded_provider, loaded_model, loaded_api_keys
    except Exception as e:
        logger.error(f"Error loading LLM preferences from {file_path}: {e}. Using defaults.")
        return defaults["last_provider"], defaults["last_model"], defaults["api_keys"]

def save_llm_preferences(provider: str, model_name: str, api_key: str):
    """Saves LLM preferences, updating the specific provider's API key."""
    file_path = os.path.join(get_root_dir(), LLM_PREFS_FILE)

    # Load existing preferences first to preserve other keys
    _, _, existing_api_keys = load_llm_preferences() # Only need existing keys

    # Update the structure
    prefs = {
        "last_provider": provider,
        "last_model": model_name,
        "api_keys": existing_api_keys # Start with existing keys
    }
    # Update or add the key for the current provider
    if provider and api_key: # Only save if provider and key are provided
         prefs["api_keys"][provider] = api_key

    try:
        with open(file_path, 'w') as f:
            json.dump(prefs, f, indent=4)
        logger.info(f"LLM preferences saved to {file_path}")
        return f"Preferences saved successfully for {provider}."
    except Exception as e:
        logger.error(f"Error saving LLM preferences to {file_path}: {e}")
        return f"Error saving preferences: {e}"

# --- Keep the original update_model_dropdown function ---
# It will be slightly modified in webui.py's change handler
def update_model_dropdown(llm_provider, api_key=None, base_url=None):
    """
    Update the model name dropdown with predefined models for the selected provider.
    (This utility function itself doesn't need changing for API key storage,
     but how it's called in webui.py will change)
    """
    import gradio as gr # Keep the import here or ensure it's globally available
    ...
    if llm_provider in model_names:
         # Ensure we return a default value that exists in the choices
